...
@login_required  #При переходе на главную страницу перенаправляет не авторизованного пользователя на форму авторизации
def index(request):
    """
    ...
    '''
    num_books_word = Book.countwords("ball")
    num_genre_and_books = num_genre + num_books_title
    '''
...
class LoanedBooksByStaffListView(LoginRequiredMixin, generic.ListView):
    ...
    """
    model = BookInstance
    template_name = 'catalog/bookinstance_list_all_borrowed_staff.html'
    paginate_by = 10
    def get_queryset(self):
